...
```{r linewidth=80}
readLines("https://raw.githubusercontent.com/jingxuag/36350/main/README.md")
```
...
source_url("https://raw.githubusercontent.com/jingxuag/36350/main/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/jingxuag/36350/main/dark_and_stormy.R")
source_url("https://raw.githubusercontent.com/jingxuag/36350/new_branch/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/jingxuag/36-350-Fork/main/check_mark.R")
```
---
...
test_that(desc = "Test that result is less than log27",            expect_lt(f("a"),log(27)))
test_that(desc = "Test that logical input work",            expect_error(f(TRUE)))
test_that(desc = "Test for expected output length (1)",         expect_length(f("m"),1))
test_that(desc = "Test that more than 1 non-letter input work",            expect_error(f("5555")))
```
...
```{r linewidth=80}
f = function(letter) {
  if (is.character(letter)==FALSE) {stop("input should be a letter") }
  if (sum(letter==c(letters,LETTERS))==0) {stop("input should be a letter") }
  if (length(letter)>1) {stop("input should be of length 1") }
  if (nchar(letter)>1) {warning("only first character will be in use") }
  letter = substr(tolower(letter),1,1)
  return(log(which(letters==letter)))
}
